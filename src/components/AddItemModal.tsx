...
export default function AddItemModal({ isOpen, onClose, existingCategories }: AddItemModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/notion/add-item', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                ...
                <div className="relative">
                  ...
                  {loading && (
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                      <span className="text-[10px] text-[--accent] font-mono animate-pulse hidden sm:inline">探测中...</span>
                    ...
                    {loading ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col items-start leading-none">
                          <span className="text-sm">正在接入...</span>
                          <span className="text-[8px] opacity-60 font-mono">CALCULATING_GRAVITY_FIELD</span>
                        </div>
                      </div>
                    ) : (
                      '存入航图'
